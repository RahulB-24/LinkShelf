      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-6 px-3 space-y-6 scrollbar-thin">
          ...
          <div className="space-y-1">
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap group relative overflow-hidden ${isActive
            ...
            >
              ...
              <span
                className={`transition-all duration-300 ease-in-out ml-3 whitespace-nowrap ${isCollapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'
                  }`}
              >
                All Bookmarks
              </span>
          ...
          <div>
            ...
            */}
            <div className={`flex items-center mb-2 h-6 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'justify-between px-3'}`}>
              <h3
                className={`text-xs font-semibold text-gray-500 dark:text-gray-500 uppercase tracking-wider overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'
                  }`}
              >
                Collections
              </h3>
              ...
              {collections.map((collection) => (
                <div key={collection.id} className="group relative">
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap overflow-hidden ${isActive
                  ...
                  >
                    ...
                    {/* Text and Count Wrapper - leave room for menu button */}
                    <div className={`flex items-center flex-1 min-w-0 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-0 max-w-0 opacity-0' : 'w-auto max-w-[170px] opacity-100'}`}>
                      <span className="truncate flex-1">{collection.name}</span>
                      <span
                        className="flex-shrink-0 text-xs bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 py-0.5 px-2 rounded-full border border-gray-200 dark:border-gray-600 group-hover:bg-white dark:group-hover:bg-gray-600 w-8 text-center ml-2 mr-6"
                      >
          ...
          <div>
            <div className={`flex items-center mb-2 h-6 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-3'}`}>
              <h3 className={`text-xs font-semibold text-gray-500 dark:text-gray-500 uppercase tracking-wider overflow-hidden transition-all duration-300 whitespace-nowrap ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'
                }`}>
                Tags
              </h3>
            ...
            <div className={`flex flex-wrap gap-2 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-3'}`}>
              {tags.map((tag) => (
                <NavLink
                  ...
                  to={`/tag/${tag.name}`}
                  className={({ isActive }) =>
                    `inline-flex items-center rounded-full text-xs font-medium border transition-colors whitespace-nowrap overflow-hidden ${isActive
        ...
        <div className="p-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
          <NavLink
            to="/settings"
            className={({ isActive }) =>
              `flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap ${isActive
                ? 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white'
                : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:text-gray-900 dark:hover:text-gray-200'
              }`
            }
